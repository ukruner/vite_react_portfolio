...
describe("ChatForm test suite", () => {
    ...
    const message = mockRef.current.value;

    beforeAll(async () => { global.fetch = vi.fn();
    ...
    //    render(<ChatForm mockRef={mockRef}/>);
    //    const button = screen.getByRole('button');
       
    //     // expect(textArea.value).toBe('Your message');
...
await vi.runAllTimersAsync();
               ...
               expect(mainStore.dispatch).toHaveBeenCalledWith({ type: 'switchers/setIsAiThinking', payload: true});
        expect(mainStore.dispatch).toHaveBeenCalledWith({ type: 'chatSlice/updateHistory', payload: { sender: 'user', text: message }})
 
        expect(mainStore.dispatch).toHaveBeenCalledWith({ type: 'switchers/setChatBotOnline', payload: true});
        vi.clearAllTimers();
     
    });
    it("received data equals to expected value", async ()=>{
        expect(result).toEqual("airesponse");
        expect(global.fetch).toHaveBeenCalledWith('http://localhost:5000/api/backend/gemini', {
            method: 'POST',
          headers: {
